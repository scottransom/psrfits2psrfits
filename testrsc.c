...
int main(int argc, char *argv[])
{
  ...
  for(i=0;i<NDATA;i++){
    ...
    if(dataval[i]<0.0){dataval[i]=rand()%100;}
    if(dataval[i]>65536.0){dataval[i]= (65536 - rand()%100);}
  }
  if(runtest()!=0){printf("Error!?\n"); return(-1);}
#endif
  ...
  for(i=0;i<NDATA;i++){
    dataval[i] = 
      ((int)(randg()*1000.0 + 20000 + rand()%20000)) % 65536;
  }
  if(runtest()!=0){printf("Error!?\n"); return(-1);}
#endif
