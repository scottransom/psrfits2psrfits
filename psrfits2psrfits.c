...
int main(int argc, char *argv[])
{
  ...
  float offset, scale, datum, packdatum, maxval;
  float *datachunk;
  FILE **infiles;
  ...
  if (argc == 1) {
    ...
    printf("           by S. Ransom, S. Chatterjee, & J. Deneva\n\n");
	...
	*pc2=0;				// terminate string
	pc1=pc2-1;
	while ((pc1 >= cmd->argv[ii]) && isdigit(*pc1)) pc1--;
	...
	*pc1=0;		     // null terminate the basefilename
	...
	if(first)
	  {
	    ...
	    pfin.sub.dat_offsets = (float *)malloc(sizeof(float) * pfin.hdr.nchan * pfin.hdr.npol);
	    pfin.sub.dat_scales = (float *)malloc(sizeof(float) * pfin.hdr.nchan * pfin.hdr.npol);
	...
	sprintf(outfilename,"%s.%0*d.fits", cmd->outfile,pfin.fnamedigits,pfin.filenum);
	...
	if(DEBUG)
	  sprintf(tform,"%dB",pfin.hdr.nsblk * pfin.hdr.nchan * pfin.hdr.npol);
	else
	...
	if(DEBUG)
	  dummy = dummy - pfin.hdr.nsblk * pfin.hdr.nchan * pfin.hdr.npol * (pfin.hdr.nbits - 8)/8;
	else
	  dummy = dummy - pfin.hdr.nsblk * pfin.hdr.nchan * pfin.hdr.npol * (pfin.hdr.nbits - cmd->numbits)/8;
	...
	while(psrfits_read_subint(&pfin,first)== 0)
	  {
	    ...
	    pfout.sub.bytes_per_subint = pfin.sub.bytes_per_subint * pfout.hdr.nbits / pfin.hdr.nbits;
		...
		for (itsamp = 0 ; itsamp < spec_per_row ; itsamp++)
		  datachunk[itsamp] = (float)(inrowdata[ichan + itsamp * pfout.hdr.nchan * pfout.hdr.npol]);
		...
		for (itsamp = 0 ; itsamp < spec_per_row ; itsamp++){
		  datum = (scale==0.0) ? 0.0 : roundf((datachunk[itsamp] - offset) / scale);
	    ...
	    fprintf(stderr,"nclipped: %d fraction clipped: %f\n",nclipped,(float)nclipped/(pfout.hdr.nchan * pfout.hdr.npol * pfout.hdr.nsblk));
